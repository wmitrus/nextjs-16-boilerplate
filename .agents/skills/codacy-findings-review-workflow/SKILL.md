...
Also honor stronger closure gates inherited from `AGENTS.md`.
...
`docs/ai/general/SECURITY_CODING_PATTERNS.md` remains the living SEC-pattern authority. Codex may use the Pattern Index plus targeted SEC sections instead of loading the full catalogue, but every finding still requires cross-reference before final classification.
