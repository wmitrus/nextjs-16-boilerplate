...
Also honor any stronger repository-wide closure requirement inherited from `AGENTS.md`; do not replace the Workflow 10 gates with a weaker set.
...
`docs/ai/general/SECURITY_CODING_PATTERNS.md` remains the living security-pattern authority. Codex may use its Pattern Index plus targeted SEC sections instead of loading the full catalogue, but every finding must still be cross-referenced before classification.
