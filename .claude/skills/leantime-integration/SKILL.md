...
They remain the semantic and operational authorities. For Claude Code, the `Context Loading` rules in this skill control how those authorities are retrieved: use targeted sections instead of the legacy mandatory full-file startup reads. This changes context-loading mechanics, not shared Leantime behavior.
