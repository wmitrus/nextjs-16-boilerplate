...
- [ ] Interface split rule (I-2):
  - [ ] Legacy `ExternalIdentityMapper` (mixed read/write) jest wycofany z read-path.
  - [ ] Read-path używa wyłącznie `InternalIdentityLookup` (lookup-only, return `string | null`).
  - [ ] Write-path create/link należy wyłącznie do `ProvisioningService`.
- [ ] Provisioning rule:
...
- [ ] Rozdzielenie read-path i write-path w auth infra
  - [ ] Wydzielić kontrakt `InternalIdentityLookup` (read-only, no side effects):
    - [ ] `findInternalUserId(provider, externalUserId): Promise<string | null>`
    - [ ] `findInternalTenantId(provider, tenantExternalId): Promise<string | null>`
  - [ ] Dodać implementację `DrizzleInternalIdentityLookup` (SELECT-only).
  ...
  - [ ] Usunąć `ensureTenantAccess()` i `resolveOrCreate*()` z flow resolvera/providera.
...
- [ ] Cleanup techniczny
  - [ ] Usunąć bootstrap membership/role z `DrizzleExternalIdentityMapper`.
  - [ ] Zastąpić użycia `ExternalIdentityMapper` przez `InternalIdentityLookup` w read-path.
...
- [ ] Unit: `InternalIdentityLookup` zwraca `string | null` i nie wykonuje INSERT/UPDATE.
...
- [ ] Read-path używa `InternalIdentityLookup`; write-path create/link jest wyłącznie w `ProvisioningService`.
...
- [ ] `src/modules/auth/infrastructure/ExternalIdentityMapper.ts`
  - [ ] zdeprecjonować/usunąć kontrakt z read-path.
- [ ] `src/modules/auth/infrastructure/InternalIdentityLookup.ts`
  - [ ] nowy kontrakt read-only (`string | null`).
- [ ] `src/modules/auth/infrastructure/drizzle/DrizzleInternalIdentityLookup.ts`
  - [ ] implementacja SELECT-only.
- [ ] `src/modules/auth/infrastructure/drizzle/DrizzleExternalIdentityMapper.ts`
  - [ ] usunąć write logic z read-path i ograniczyć użycie wyłącznie do migracji/compat (do finalnego usunięcia).
...
- [ ] `ExternalIdentityMapper` nie jest używany w runtime read-path.
...
- [ ] I-3 Canonical Role Naming in Seeds
  - [ ] Kanoniczne nazwy ról wewnętrznych: wyłącznie `owner` i `member`.
  - [ ] Dev/test seed: zmiana `admin` -> `owner`.
  - [ ] Usunąć założenia/test fixtures oczekujące roli `admin` jako internal canonical role.
  - [ ] Dodać migrację/backfill dla środowisk dev/test z istniejącą rolą `admin` (mapowanie membership do `owner`).
- [ ] Runbook/docs update
...
- [ ] DB integration: seed/provisioning tworzy wyłącznie role `owner`/`member` (brak canonical `admin`).
...
- [ ] Internal canonical role names: `owner`/`member` (rola `admin` może istnieć tylko jako alias claim mapowany do `owner`).
