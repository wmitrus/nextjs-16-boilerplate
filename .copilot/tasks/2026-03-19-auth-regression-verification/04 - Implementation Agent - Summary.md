...
- blockers: Phase 0 is no longer blocked on runner support, incomplete-user test-flow design, Chromium host dependencies, stale local repo-owned Next.js dev state, or E2E rate limiting on provisioning-status probes; only optional server-log visibility remains as a workflow/observability concern
