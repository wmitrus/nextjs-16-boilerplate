...
- Current Run Scope: Phase 0a runner alignment, AF-06 / AF-07 rerunnable incomplete-user flow implementation, AF-01 hosted Clerk verification harness fix, Phase 2 contract alignment plus explicit AF-08 / AF-09 coverage, package-level auth-matrix phase scripts backed by Playwright title tags, auth-matrix package-run stabilization for true list mode plus local/CI runtime reliability, and Phase 3 cookie/source-of-truth Playwright coverage
- Status: COMPLETED
...
- code changes made: added backend-mode validation in `load-env.mjs`; updated `run-scenario.mjs` to branch setup by `E2E_BACKEND_MODE`, preserve the existing PGlite file-backed flow, and use `db:test:up` plus `node scripts/db-ops.mjs test reset --force` for container mode while keeping Playwright invocation unchanged; added optional incomplete-user helper support in `e2e/clerk-auth.ts`; implemented rerunnable AF-06 / AF-07 single-mode flows in `e2e/provisioning-runtime.spec.ts`; updated `e2e/auth.spec.ts` so hosted sign-up waits deterministically for either Clerk verify-email or bootstrap and no longer assumes the bootstrap request must be flagged as a navigation request; then updated `e2e/provisioning-runtime.spec.ts` again so AF-05 creates completed single-user state inside the run, AF-06 / AF-07 assert the API-layer onboarding-required code and visible onboarding UI, the local bootstrap-request helper no longer depends on `request.isNavigationRequest()`, the incomplete-user route assertions target `/onboarding` rather than requiring preserved `redirect_url`, AF-08 / AF-09 are explicitly covered, and the completed-user setup helper is idempotent across serial tests without embedding extra readiness probes; then added Playwright title tags `@auth-matrix-phase1`, `@auth-matrix-phase2`, and `@auth-matrix-phase3` plus `package.json` scripts `e2e:auth-matrix`, `e2e:auth-matrix:phase1`, `e2e:auth-matrix:phase2`, `e2e:auth-matrix:phase3`, and `e2e:auth-matrix:ci`; finally updated `run-scenario.mjs` to preserve true Playwright list mode, normalize forwarded args, prefer healthy local server reuse, and clean up stale repo-local Next.js dev processes plus `.next/dev/lock` when the configured base URL is unreachable, split Phase 2 package execution into returning-state and direct-entry sub-slices, updated `src/security/middleware/with-rate-limit.ts` to bypass rate limiting during E2E runs, and added Phase 3 Playwright helpers to capture cookie state and redirect-response evidence for AF-12 through AF-15
- tests or supporting files updated: `src/security/middleware/with-rate-limit.test.ts`
- focused validation executed: syntax checks, backend-mode helper validation, env validation, `--list` runner validation for both backend modes, Playwright spec listing after the AF-06 / AF-07 additions, targeted Chromium reruns for AF-01 and AF-05 / AF-06 / AF-07, package-level validation for `e2e:auth-matrix:phase1` and `e2e:auth-matrix:phase2`, wrapper list-mode validation through `pnpm e2e:auth-matrix:phase1 -- --list`, full package-level `pnpm e2e:auth-matrix` reruns while triaging local Next.js and rate-limit issues, Phase 3 wrapper validation through `pnpm e2e:auth-matrix:phase3 -- --list`, full Phase 3 Chromium execution, and a focused stale-cookie rerun for AF-15

## Files Changed

- production files: `scripts/e2e/load-env.mjs`, `scripts/e2e/run-scenario.mjs`, `e2e/clerk-auth.ts`, `e2e/provisioning-runtime.spec.ts`, `e2e/auth.spec.ts`, `package.json`, `src/security/middleware/with-rate-limit.ts`
- test files: `src/security/middleware/with-rate-limit.test.ts`
...
- blockers: Phase 0 is no longer blocked on runner support, incomplete-user test-flow design, Chromium host dependencies, stale local repo-owned Next.js dev state, or E2E rate limiting on provisioning-status probes; optional server-log visibility remains a workflow/observability concern
...
- Date: 2026-03-21
- Trigger: Auth-matrix package-run stabilization
- Summary of change: fixed wrapper-forwarded `--list` so it remains non-executing, hardened the universal runner against stale repo-local Next.js dev processes and `.next/dev/lock`, split the Phase 2 package entrypoint into smaller stable slices, bypassed rate limiting during E2E auth verification, and validated the final `pnpm e2e:auth-matrix` package flow end to end
- Sections refreshed: task context, scope handled, actions performed, files changed, behavior change summary, validation performed, open questions/blockers, update log

### Update Entry

- Date: 2026-03-21
- Trigger: Phase 3 cookie/source-of-truth coverage implementation
- Summary of change: added `@auth-matrix-phase3` Playwright coverage and a dedicated package script for AF-12 through AF-15, exposed a stale-cookie AF-15 regression, fixed the product behavior by keeping `/users` DB-authoritative in edge mode, and validated the result with a passing focused stale-cookie rerun plus a green full Phase 3 package run
- Sections refreshed: task context, scope handled, actions performed, validation performed, intentional non-changes, blockers, update log
