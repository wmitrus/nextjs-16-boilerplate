...
Any future change for New Relic deployment/change tracking in this repository must treat successful production deployment as the source of truth for a production deployment marker.
...
- `release.published` must not be treated as proof of production deployment success in the current repository topology.
- A production deployment marker must only be emitted from:
  - the successful path inside `prod-deploy.yml`, or
  - a downstream workflow triggered by successful completion of `prod-deploy.yml`
...
- If semantic version is required, the implementation must prove that the version is available on the deployment-success path without race conditions or ambiguity.
- Do not assume semantic-release output is naturally available to deploy-triggered workflows.
...
- Marker emitted from `release.published` while production deploy remains separate
...
- Keep workflow ownership boundaries explicit: release manages versioning metadata, production deploy manages rollout truth.
...
- Final implementation choice is blocked on product decision about marker identity requirements: semver, SHA, or both.
