...
- For normal JSON App Router route handlers, use `src/shared/lib/api/response-service.ts` helpers and `src/shared/lib/api/with-error-handler.ts`. This is enforced by `response-service.guard.test.ts`: a route that diverges must be added to its `EXEMPT_ROUTES` with a written reason, so the divergence is a recorded decision rather than a silent one. See SEC-38.
...
Use it to avoid reintroducing broad cleanup classes such as:

- business logic leaking into `shared/*` or delivery layers
- dynamic bracket dispatch and repeated dynamic object mutation chains
- repeated raw script fs access instead of shared sink-confined helpers
- broad refactors mixed into behavior changes
- phase-close validation being skipped silently
...
For App Router route handlers, this also means every UUID path segment must be parsed
with `z.uuid()` before DB use. A presence check such as `if (!params.id)` is not
validation. Add a malformed-ID unit test proving `400` is returned before DB or
repository calls.
