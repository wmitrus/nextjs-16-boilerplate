...
- write real credential-shaped values (keys, tokens, passwords, license keys, API keys) or realistic-looking credential examples into committed markdown files — always replace with `[REDACTED]`, `[hash-prefix]`, or another neutral placeholder even when the value is technically public or browser-visible; gitleaks scans committed markdown and will fail the `security-scan` CI workflow
