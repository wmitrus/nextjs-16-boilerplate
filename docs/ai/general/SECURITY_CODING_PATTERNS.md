...
| SEC-49 | Trust boundaries          | Runtime-untrusted input (persisted, serialized, file-loaded, or connector-loaded) typed directly as a trusted internal shape instead of validated from `unknown`                                                                                          | Real risk → generic rule                                                                                                                                | Any boundary that loads/parses/deserializes data the process did not just produce and validate itself |
...
## SEC-49 — Runtime-Untrusted Input Must Enter As `unknown`, Not As An Asserted Trusted Type

**ID**: SEC-49
**Category**: Trust boundaries
**Classification**: Real risk (generic rule)
**Affected contexts**: Any boundary that loads, parses, or deserializes data the current process did not just produce and validate itself — persisted files, serialized artifacts, connector/adapter responses, and similar runtime-untrusted sources

### Risk

Persisted, serialized, file-loaded, connector-loaded, or otherwise
runtime-untrusted input must enter the trust boundary as `unknown`, be
runtime-validated once at that boundary, and only the validated output may
flow into typed internal helpers.

TypeScript's `as` assertion is a compile-time-only claim; it does not run any
check. Asserting a `JSON.parse()` result (or any other runtime-untrusted
value) directly as a trusted internal type makes the type system hide real
runtime possibilities — `null`, a wrong primitive type, a missing field, a
malformed array entry, an unexpected object shape — from every function that
receives the "typed" value afterward. Each downstream helper then trusts a
shape nothing has actually confirmed.

### Dangerous Pattern

```typescript
// DANGEROUS: assertion, not validation — a malformed file crashes or
// silently misbehaves somewhere downstream instead of failing at the
...
**DO NOT** make runtime-untrusted input appear trusted by direct assertion,
such as `JSON.parse(value) as SomeTrustedArtifact`.
**DO** accept runtime-untrusted input as `unknown` and validate it once at
the trust boundary before any typed internal code touches it.
**DO** apply this to every source of runtime-untrusted input, not only
network requests — a file on disk, a persisted artifact, or a
connector/adapter response is exactly as untrusted as an HTTP body until it
has been validated.
