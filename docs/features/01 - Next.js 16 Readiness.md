...
export default withSentryConfig(nextConfig, {
  // Sentry source map upload and tunnel configuration
  // See: https://docs.sentry.io/platforms/javascript/guides/nextjs/manual-setup/
});
```

Note: The config is exported via `withSentryConfig` wrapper for Sentry integration (source maps, tunnel route, Vercel Cron monitors). The `serverExternalPackages` list marks native/CJS packages that must run in Node.js and be excluded from the edge bundle.
